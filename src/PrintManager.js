...
const renderPayslipToHtml = (payslipData) => {
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl border border-gray-200 font-sans max-w-2xl mx-auto text-gray-800">
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-y-1 text-sm">
            <p>Basic Salary:</p>
            <p className="text-right font-semibold">{formatCurrency(data.basicSalary)}</p>
            {data.costOfLivingAllowance > 0 && (
              <>
                <p>Cost of Living Allowance:</p>
                <p className="text-right font-semibold">{formatCurrency(data.costOfLivingAllowance)}</p>
              </>
            )}
            <div className="col-span-2 border-t mt-2 pt-2 flex justify-between items-center">
              ...
              <p className="text-right font-bold text-lg text-indigo-600">{formatCurrency(data.grossSalary)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="grid grid-cols-2 gap-y-1 text-sm">
            ...
            <p className="text-right">{formatCurrency(data.sssContribution)}</p>
            <p>Philhealth Contribution:</p>
            <p className="text-right">{formatCurrency(data.philhealthContribution)}</p>
            <p>Pag-IBIG Contribution:</p>
            <p className="text-right">{formatCurrency(data.pagibigContribution)}</p>
            <p>CEAP Contribution:</p>
            <p className="text-right">{formatCurrency(data.ceapContribution)}</p>

            {data.tithings > 0 && <>
              <p>Tithings:</p>
              <p className="text-right">{formatCurrency(data.tithings)}</p>
            </>}

            {data.canteen > 0 && <>
              <p>Canteen:</p>
              <p className="text-right">{formatCurrency(data.canteen)}</p>
            </>}

            {data.pagibigLoanSTL > 0 && <>
              <p>Pag-IBIG Loan-STL:</p>
              <p className="text-right">{formatCurrency(data.pagibigLoanSTL)}</p>
            </>}

            {data.pagibigLoanCL > 0 && <>
              <p>Pag-IBIG Loan-CL:</p>
              <p className="text-right">{formatCurrency(data.pagibigLoanCL)}</p>
            </>}

            {data.sssLoan > 0 && <>
              <p>SSS Loan:</p>
              <p className="text-right">{formatCurrency(data.sssLoan)}</p>
            </>}

            {data.personalLoan > 0 && <>
              <p>Personal Loan:</p>
              <p className="text-right">{formatCurrency(data.personalLoan)}</p>
            </>}

            {data.cashAdvance > 0 && <>
              <p>Cash Advance:</p>
              <p className="text-right">{formatCurrency(data.cashAdvance)}</p>
            </>}

            {(data.otherDeductions || []).map((ded, index) =>
              ded.amount > 0 && (
            ...
            <div className="col-span-2 border-t mt-2 pt-2 flex justify-between items-center">
              ...
              <p className="text-right font-bold">{formatCurrency(data.totalDeductions)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="text-xl font-bold border-t-2 pt-4 mt-6 flex justify-between items-center text-gray-800">
          <span>NET PAY:</span>
          <span className="text-green-700">{formatCurrency(data.netSalary)}</span>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-8 text-center text-sm">
          ...
          <div>
            <div className="signature-line-placeholder"></div>
            <p className="font-semibold">{data.bookkeeperName}</p>
  ...
  return ReactDOMServer.renderToString(<PayslipDisplayForPrint payslipData={modifiedPayslipData} />);
};

const PrintManager = forwardRef(({
  payslip,
  employees,
  payslipDetails,
  setPayslipDetails,
  ...
  startDate,
  endDate,
  isPayslipPreview = false,
  hideButton = false, // Add this new prop
}, ref) => {
  ...
  const [printOption, setPrintOption] = useState(isPayslipPreview ? 'current' : 'all');

  // Expose printAllPayslips method to parent
  useImperativeHandle(ref, () => ({
    printAllPayslips: (payslipsToPrint) => {
      if (!payslipsToPrint || payslipsToPrint.length === 0) {
        setShowNoPayslipAlert(true);
        return;
      }
      executePrintAll(payslipsToPrint);
    }
  }));

  const executePrintAll = (payslipsToGenerate) => {
  ...
  const executePrintCurrent = (payslipToGenerate) => {
    const printWindow = window.open('', '_blank');
    if (!printWindow) {
      setShowPopupBlockerAlert(true);
      return;
    ...
    const html = renderPayslipToHtml(payslipToGenerate);

    const payslipHtml = `
      <div class="page-container">
        <div class="employee-payslip-section">
          <div class="payslip-copy">${html}</div>
          <div class="payslip-copy">${html}</div>
        </div>
      </div>
    `;
  ...
  // Handlers for modal open/close and execute print
  ...
  const executePrint = () => {
    if (isPayslipPreview || printOption === 'current') {
      if (!payslip) {
        setShowNoPayslipAlert(true);
        return;
      }
      executePrintCurrent(payslip);
    } else if (printOption === 'all') {
      if (!payslipHistory || payslipHistory.length === 0) {
        setShowNoPayslipAlert(true);
        return;
      }
      executePrintAll(payslipHistory);
    }
  ...
  return (
    <>
      ...
      {!hideButton && (
        <button onClick={handleOpenPrintOptions} className="px-4 py-2 bg-indigo-600 text-white rounded-md">
          Print
        </button>
      )}
      ...
      <PrintOptionsModal
        ...
        endDate={endDate}
        setEndDate={() => {}}
        isPayslipPreview={isPayslipPreview}
      />

      <ConfirmationModal
        show={showNoPayslipAlert}
        onClose={() => setShowNoPayslipAlert(false)}
        title="No Payslip Available"
        message="No payslip has been generated yet for the selected range."
      />

      <ConfirmationModal
        show={showPopupBlockerAlert}
        onClose={() => setShowPopupBlockerAlert(false)}
        title="Popup Blocker Detected"
        message="Please enable popups for this site to print the payslip."
      />
    </>
  );
});
